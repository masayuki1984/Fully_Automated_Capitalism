...
DOWNLOAD_SAVE_DIR = BASE_DIR + "../data/tosho_fund_and_others_stock_prices/"

def retrieve_csv_file(url):
    res = requests.get(url, auth=(KABU_PLUS_ID, KABU_PLUS_PW))
    if res.status_code != 200:
        log.error('ステータスコードが200以外')
        slack.notify(text="Fund and Others取得処理：異常終了")
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    log.info('Fund and Others取得処理 : 開始')
    dt = datetime.date.today().strftime("%Y%m%d")
    file_name = 'tosho-fund-and-others-stock-prices_' + dt + '.csv'
    URL = 'https://csvex.com/kabu.plus/csv/tosho-fund-and-others-stock-prices/daily/' + file_name
    log.info('CSVファイル取得処理（FUnd and Othres）ファイル名:' + file_name)
    data = retrieve_csv_file(URL)
    to_csv(data, DOWNLOAD_SAVE_DIR + file_name)
    log.info('Fund and Others取得処理 : 終了')
    slack.notify(text="Fund and Others取得処理：正常終了")
    sys.exit(0)
...
TARGET_DIR = path_config.get('csv_path', 'data_base') + path_config.get('csv_path', 'japan_all_stock_data')
FILE_NAME_PREFIX = 'japan-all-stock-data_'
...
if __name__ == '__main__':
    ...
    try:
        ...
        log.error('投資指標データ　CSVファイル圧縮月次処理失敗')
        sys.exit(1)

    log.info('投資指標データ　CSVファイル圧縮月次処理成功')
    slack.notify(text="投資指標データ CSVファイル圧縮月次処理(" + target_month + ")：正常終了")
    sys.exit(0)

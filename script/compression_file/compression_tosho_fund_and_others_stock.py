...
TARGET_DIR = path_config.get('csv_path', 'data_base') + path_config.get('csv_path', 'tosho_fund_and_others_stock_prices')
FILE_NAME_PREFIX = 'tosho-fund-and-others-stock-prices_'
...
if __name__ == '__main__':
    ...
    try:
        ...
        log.error('Fund and Others CSVファイル圧縮月次処理失敗')
        sys.exit(1)

    log.info('Fund and Others CSVファイル圧縮月次処理成功')
    slack.notify(text="Fund and Others CSVファイル圧縮月次処理(" + target_month + ")：正常終了")
    sys.exit(0)

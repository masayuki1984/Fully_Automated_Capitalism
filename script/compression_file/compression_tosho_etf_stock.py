...
TARGET_DIR = path_config.get('csv_path', 'data_base') + path_config.get('csv_path', 'tosho_etf_stock_prices')
FILE_NAME_PREFIX = 'tosho-etf-stock-prices_'
...
if __name__ == '__main__':
    ...
    try:
        ...
        log.error('ETF CSVファイル圧縮月次処理失敗')
        sys.exit(1)

    log.info('ETF CSVファイル圧縮月次処理成功')
    slack.notify(text="ETF CSVファイル圧縮月次処理(" + target_month + ")：正常終了")
    sys.exit(0)
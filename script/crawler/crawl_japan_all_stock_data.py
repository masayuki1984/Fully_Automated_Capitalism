...
DOWNLOAD_SAVE_DIR = path_config.get('csv_path', 'data_base') + path_config.get('csv_path', 'japan_all_stock_data')
...
class CSVFile:
    ...
    def retrieve_csv_file(self, url):
        ...
        if res.status_code != 200:
            ...
            slack.notify(text="投資指標データ取得処理：異常終了")
            sys.exit(1)
...
if __name__ == '__main__':
    ...
    log.info('投資指標データ取得処理 : 開始')
    # CSVファイルを取り扱うクラスの作成
    csv_file = CSVFile(KABU_PLUS_ID, KABU_PLUS_PW)

    file_name = 'japan-all-stock-data_' + file_name_date_part + '.csv'
    URL = 'https://csvex.com/kabu.plus/csv/japan-all-stock-data/daily/' + file_name
    log.info('CSVファイル取得処理（投資指標データ）ファイル名:' + file_name)
    data = csv_file.retrieve_csv_file(URL)
    csv_file.to_csv(data, DOWNLOAD_SAVE_DIR + file_name)
    log.info('投資指標データ取得処理 : 終了')
    slack.notify(text="投資指標データ取得処理：正常終了")
    sys.exit(0)
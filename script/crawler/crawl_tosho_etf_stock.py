...
DOWNLOAD_SAVE_DIR = path_config.get('csv_path', 'data_base') + path_config.get('csv_path', 'tosho_etf_stock_prices')
...
class CSVFile:
    ...
    def retrieve_csv_file(self, url):
        ...
        if res.status_code != 200:
            ...
            slack.notify(text="ETF取得処理：異常終了")
            sys.exit(1)
...
if __name__ == '__main__':
    ...
    URL = 'https://csvex.com/kabu.plus/csv/tosho-etf-stock-prices/daily/' + file_name
    log.info('CSVファイル取得処理（ETF）ファイル名:' + file_name)
    data = csv_file.retrieve_csv_file(URL)
    csv_file.to_csv(data, DOWNLOAD_SAVE_DIR + file_name)
    log.info('ETF取得処理 : 終了')
    slack.notify(text="ETF取得処理：正常終了")
    sys.exit(0)
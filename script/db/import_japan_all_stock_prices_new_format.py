...
CSV_FILE_DIR = path_config.get('csv_path', 'data_base') + path_config.get('csv_path', 'japan_all_stock_prices_new_format')
...
if __name__ == '__main__':
    ...
    log.info('日本株全銘柄(新フォーマット)テーブルインポート処理 : 開始')
    ...
    with open (CSV_FILE_DIR + file_name) as csvfile:
        ...
        try:
            ...
            for row in reader:
                ...
                volume = row[13] if row[13] != '-' else 'null'
                volume_ratio = row[14] if row[14] != '-' else 'null'
                trading_value = row[15] if row[15] != '-' else 'null'
                market_capitalization = row[16] if row[16] != '-' else 'null'
                price_range_lower_limit = row[17] if row[17] != '-' else'null'
                price_range_upper_limit = row[18] if row[18] != '-' else 'null'
                vwap = row[12] if row[12] != '-' else 'null'
                high_price_date = row[19] if row[19] != '-' else 'null'
                low_price_date = row[22] if row[22] != '-' else 'null'
                yearly_high_price = row[20] if row[20] != '-' else 'null'
                yearly_low_price = row[23] if row[23] != '-' else 'null'
                yearly_high_price_deviation_rate = row[21] if row[21] != '-' else 'null'
                yearly_low_price_deviation_rate = row[24] if row[24] != '-' else 'null'
                cursor.execute('''INSERT INTO %s.%s (security_code, dt, company_name,
                               ...
                               day_before_ratio, day_before_ratio_percentage, last_day_closing_price, volume, volume_ratio,
                               trading_value, market_capitalization, price_range_lower_limit, price_range_upper_limit,
                               vwap, high_price_date, low_price_date, yearly_high_price, yearly_low_price, 
                               yearly_high_price_deviation_rate, yearly_low_price_deviation_rate)
                               ...
                               ''' % (DB_DATABASE, TABLE, security_code, dt, company_name, stock_exchange_code, industry_type, opening_price,
                                      ...
                                      last_day_closing_price, volume, volume_ratio, trading_value, market_capitalization,
                                      price_range_lower_limit, price_range_upper_limit, vwap, high_price_date, low_price_date,
                                      yearly_high_price, yearly_low_price, yearly_high_price_deviation_rate,
                                      yearly_low_price_deviation_rate))
        ...
        try:
            ...
            slack.notify(text="日本株全銘柄(新フォーマット)テーブルインポート\n" + result_word)
        except mysql.connector.Error as e:
            log.error(e)
            conn.close()

        log.info('日本株全銘柄(新フォーマット)テーブルインポート処理 : 終了')
        conn.close()

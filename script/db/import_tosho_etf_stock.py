...
TABLE = 'tosho_etf_stock_prices'
CSV_FILE_DIR = path_config.get('csv_path', 'data_base') + path_config.get('csv_path', 'tosho_etf_stock_prices')
...
if __name__ == '__main__':
    ...
    file_name = 'tosho-etf-stock-prices_' + file_name_date_part + '.csv'
    ...
    with open (CSV_FILE_DIR + file_name) as csvfile:
        ...
        try:
            ...
            for row in reader:
                ...
                query = '''INSERT INTO %s.%s (security_code, dt, etf_name, 
        ...
        try:
            conn = mysql.connector.connect(user=DB_USER, password=DB_PASSWORD, host=DB_HOST, database=DB_DATABASE)
            cursor = conn.cursor()
            count_query = """SELECT dt, COUNT(*) FROM %s.%s WHERE dt = '%s' GROUP BY dt""" % (
            DB_DATABASE, TABLE, TODAY)
